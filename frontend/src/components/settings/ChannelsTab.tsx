...
export default function ChannelsTab() {
  ...
  const createMutation = useMutation({
    mutationFn: () => createChannel({
      name,
      type: 'evolution',
      config: { instance, apiKey: evolutionApiKey, webhookSecret: randomHex(16) },
    }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setNewOpen(true)} className="flex items-center gap-1.5 bg-brand-600 hover:bg-brand-500 text-white text-sm px-3 py-1.5 rounded-lg">
          ...
          <div key={c.id} className="bg-slate-800 rounded-xl p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-slate-500 capitalize">
                {c.type} • <span className={c.status === 'connected' ? 'text-emerald-400' : c.status === 'error' ? 'text-red-400' : 'text-amber-400'}>{c.status}</span>
              </div>
            </div>
      ...
      <Modal open={newOpen} onClose={() => setNewOpen(false)} title="Novo canal Evolution">
        <div className="space-y-3">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome (ex: WhatsApp Principal)" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100" />
          <input value={instance} onChange={(e) => setInstance(e.target.value)} placeholder="Nome da instância no Evolution (ex: flowcrm-main)" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100" />
          <input value={evolutionApiKey} onChange={(e) => setEvolutionApiKey(e.target.value)} placeholder="API key global do Evolution" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100" />
          <div className="flex justify-end gap-2">
            <button onClick={() => setNewOpen(false)} className="px-3 py-1.5 text-sm text-slate-400">Cancelar</button>
            <button
              onClick={() => createMutation.mutate()}
              disabled={!name || !instance || !evolutionApiKey || createMutation.isPending}
