...
import { getContact, updateContact, listContacts } from '@/api/contacts';
import { getCompany, updateCompany, listCompanies } from '@/api/companies';
    ...
    <div className="px-3 pb-3">
      ...
      {(lead.contactId || lead.companyId) && (
        <FieldRow label={isCompany ? 'Nome da empresa' : 'Nome'}>
          <InlineContactSearch
            currentName={isCompany ? (lead.company?.name ?? '') : (lead.contact?.name ?? '')}
            mode={isCompany ? 'company' : 'contact'}
            onSelect={(id) => {
              if (isCompany) mut.mutate({ companyId: id });
              else mut.mutate({ contactId: id });
            }}
          />
        </FieldRow>
      )}
